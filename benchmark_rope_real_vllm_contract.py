...
RESHAPE_AND_CACHE_PATH = None
RESHAPE_AND_CACHE_ERROR = None
reshape_and_cache = None
for _path, _loader in (
    (
        "vllm._C + torch.ops._C_cache_ops.reshape_and_cache",
        lambda: (
            __import__("vllm._C"),
            torch.ops._C_cache_ops.reshape_and_cache,
        )[1],
    ),
    ("vllm._custom_ops.reshape_and_cache", lambda: __import__("vllm._custom_ops", fromlist=["reshape_and_cache"]).reshape_and_cache),
    ("torch.ops._C_cache_ops.reshape_and_cache", lambda: torch.ops._C_cache_ops.reshape_and_cache),
    ("vllm._custom_ops module attr", lambda: __import__("vllm", fromlist=["_custom_ops"])._custom_ops.reshape_and_cache),
):
    try:
        reshape_and_cache = _loader()
        RESHAPE_AND_CACHE_PATH = _path
        RESHAPE_AND_CACHE_ERROR = None
        break
    except Exception as exc:
        RESHAPE_AND_CACHE_ERROR = f"{_path}: {exc!r}"
...
if HAS_TORCH_TRITON:
    ...
    def _vllm_write_kernel(
        ...
        rem = offsets - token * token_stride
        head = rem // head_dim
        dim = rem - head * head_dim
        slot = tl.load(slot_ptr + token, mask=mask, other=0)
        cache_block = slot // block_size
        block_offset = slot - cache_block * block_size

        key_offset = (((cache_block * heads + head) * (head_dim // x) + (dim // x)) * block_size + block_offset) * x + (dim % x)
        value_offset = ((cache_block * heads + head) * head_dim + dim) * block_size + block_offset
        k = tl.load(k_ptr + offsets, mask=mask, other=0.0)
        v = tl.load(v_ptr + offsets, mask=mask, other=0.0)
        tl.store(k_cache_ptr + key_offset, k, mask=mask)
        tl.store(v_cache_ptr + value_offset, v, mask=mask)

    @triton.jit
    def _vllm_rope_kv_write_kernel(
        k_ptr,
        v_ptr,
        cos_ptr,
        sin_ptr,
        ...
        kout = tl.where((dim % 2) == 1, odd_out, even_out)
        vv = tl.load(v_ptr + offsets, mask=mask, other=0.0)
        tl.store(k_cache_ptr + key_offset, kout, mask=mask)
        tl.store(v_cache_ptr + value_offset, vv, mask=mask)
...
def build_cos_sin(positions: torch.Tensor, head_dim: int, dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
    inv_freq = 1.0 / (10000 ** (torch.arange(0, head_dim, 2, device="cuda", dtype=torch.float32) / head_dim))
    freqs = torch.outer(positions.float(), inv_freq)
    return freqs.cos().to(dtype=dtype).contiguous(), freqs.sin().to(dtype=dtype).contiguous()
...
def vllm_layout_cache(num_blocks: int, kv_heads: int, head_dim: int, block_size: int, dtype: torch.dtype) -> tuple[torch.Tensor, torch.Tensor]:
    x = 16 // torch.tensor([], dtype=dtype).element_size()
    key_cache = torch.empty(num_blocks, kv_heads, head_dim // x, block_size, x, device="cuda", dtype=dtype)
    value_cache = torch.empty(num_blocks, kv_heads, head_dim, block_size, device="cuda", dtype=dtype)
    return key_cache, value_cache


def vllm_layout_flat_views(k_cache: torch.Tensor, v_cache: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    num_blocks, kv_heads, head_chunks, block_size, x = k_cache.shape
    head_dim = head_chunks * x
    flat_k = (
        k_cache.permute(0, 3, 1, 2, 4)
        .contiguous()
        .view(num_blocks * block_size, kv_heads, head_dim)
    )
    flat_v = v_cache.permute(0, 3, 1, 2).contiguous().view(num_blocks * block_size, kv_heads, head_dim)
    return flat_k, flat_v


def vllm_layout_write(
    k: torch.Tensor,
    v: torch.Tensor,
    slots: torch.Tensor,
    k_cache: torch.Tensor,
    ...
    x = k_cache.shape[4]
    grid = (triton.cdiv(total_elems, block),)
    _vllm_write_kernel[grid](k, v, slots, k_cache, v_cache, total_elems, heads, head_dim, block_size, x, BLOCK=block)


def fused_vllm_layout_write(
    k: torch.Tensor,
    v: torch.Tensor,
    cos: torch.Tensor,
    sin: torch.Tensor,
    ...
    x = k_cache.shape[4]
    grid = (triton.cdiv(total_elems, block),)
    _vllm_rope_kv_write_kernel[grid](
        k, v, cos, sin, slots, k_cache, v_cache, total_elems, heads, half_dim, head_dim, block_size, x, BLOCK=block
    )
...
def build_contract(case: ContractCase) -> dict[str, object]:
    ...
    return {
        ...
        "layout": {
            "cache_shape": {
                "key_cache": [total_logical_blocks, case.kv_heads, case.head_dim // 8, case.block_size, 8],
                "value_cache": [total_logical_blocks, case.kv_heads, case.head_dim, case.block_size],
            },
            "slot_formula": "vLLM paged layout: key[block, kv_head, head_dim // x, block_offset, x], value[block, kv_head, head_dim, block_offset]",
            "assumes_actual_vllm_cache_writer": True,
        },
    }
...
def dry_run(output: Path) -> None:
    ...
    payload = {
        "benchmark": "rope_real_vllm_contract",
        "dry_run": True,
...
def run_case(case: ContractCase, warmup: int, repeats: int) -> dict[str, object]:
    if reshape_and_cache is None:
        raise RuntimeError(f"vLLM reshape_and_cache is unavailable: {RESHAPE_AND_CACHE_ERROR}")
    ...
    if HAS_VLLM:
        ...
        rope_provider = "vllm_rotary_embedding"
    else:
    ...
    def inductor_then_write():
        ...
        vllm_layout_write(tmp, v, slots, k_cache, v_cache, case.triton_block)
    ...
    return {
        ...
        "reshape_and_cache_path": RESHAPE_AND_CACHE_PATH,
        "vllm_then_reshape_and_cache_us": None if vllm_us is None else round(vllm_us, 3),
        ...
        "fused_vs_vllm_reshape_and_cache": fused_vs_vllm,
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark RoPE + KV write against real vLLM reshape_and_cache.")
...
def main() -> None:
    ...
    if reshape_and_cache is None:
        raise RuntimeError(f"vLLM reshape_and_cache is required for the runtime benchmark: {RESHAPE_AND_CACHE_ERROR}")
    ...
    with vllm_config_context():
        for idx, case in enumerate(cases, 1):
            ...
            print(
                ...
                f"vs_vllm={format_ratio(row['fused_vs_vllm_reshape_and_cache'])} "
                f"oracle={row['oracle']}",
                flush=True,
            )
