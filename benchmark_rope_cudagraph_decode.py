...
def dry_run(output: Path) -> None:
    ...
    payload = {
        "benchmark": "rope_cudagraph_decode",
        "dry_run": True,
...
def run_case(case: ContractCase, warmup: int, repeats: int) -> dict[str, object]:
    ...
    baseline_error = None
    try:
        baseline_us = cuda_time_us(baseline_eager, warmup, repeats)
    except RuntimeError as exc:
        if "undefined Tensor" not in str(exc):
            raise
        baseline_error = "vLLM key-only RoPE is unavailable in this installed vLLM API"
        torch.cuda.synchronize()
    fused_us = cuda_time_us(fused_eager, warmup, repeats)
    speedup_eager = None if baseline_us is None else round(baseline_us / fused_us, 4)
    ...
    if baseline_us is not None:
        try:
            k_graph = k.clone()
            v_graph = v.clone()
            positions_graph = positions.clone()
            slots_graph = slots.clone()
            cos_graph = cos.clone()
            sin_graph = sin.clone()
            baseline_tmp_graph = torch.empty_like(k_graph)
            k_cache_graph_base = torch.empty_like(k_cache)
            v_cache_graph_base = torch.empty_like(v_cache)
            k_cache_graph_fused = torch.empty_like(k_cache)
            v_cache_graph_fused = torch.empty_like(v_cache)

            def baseline_static():
                if emb is not None:
                    baseline_tmp_graph.copy_(k_graph)
                    emb.forward_cuda(positions_graph, baseline_tmp_graph, None)
                else:
                    baseline_tmp_graph.copy_(rope_ref(k_graph, cos_graph, sin_graph))
                contract_write(
                    baseline_tmp_graph,
                    v_graph,
                    slots_graph,
    ...
    return {
        ...
        "eager_baseline_us": None if baseline_us is None else round(baseline_us, 3),
        "eager_fused_us": round(fused_us, 3),
        ...
        "note": "eager speedup includes launch overhead; graph speedup is replay-only",
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark decode RoPE + KV write with eager and CUDA Graph replay.")
...
def main() -> None:
    ...
    with vllm_config_context():
        for idx, case in enumerate(cases, 1):
            ...
            print(
                ...
                f"eager={format_ratio(row['speedup_eager'])} "
                f"graph={format_ratio(row['speedup_graph'])} "
                f"oracle={row['oracle']}",
                flush=True,
            )
    ...
    payload = {
        ...
        "non_claims": [
            ...
            "contract writer baseline, not real vLLM reshape_and_cache",
